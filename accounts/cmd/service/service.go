...
var fs = flag.NewFlagSet("accounts", flag.ExitOnError)
var debugAddr = fs.String("debug-addr", ":3000", "Debug and metrics listen address")
var httpAddr = fs.String("http-addr", ":3001", "HTTP listen address")
var grpcAddr = fs.String("grpc-addr", ":3002", "gRPC listen address")
var thriftAddr = fs.String("thrift-addr", ":3003", "Thrift listen address")
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		endpoint, err := zipkingo.NewEndpoint("accounts", "localhost:80")
		if err != nil {
			logger.Log("err", err)
			os.Exit(1)
		}
