...
var fs = flag.NewFlagSet("messages", flag.ExitOnError)
var debugAddr = fs.String("debug-addr", ":6000", "Debug and metrics listen address")
var httpAddr = fs.String("http-addr", ":6001", "HTTP listen address")
var grpcAddr = fs.String("grpc-addr", ":6002", "gRPC listen address")
var thriftAddr = fs.String("thrift-addr", ":6003", "Thrift listen address")
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		endpoint, err := zipkingo.NewEndpoint("messages", "localhost:80")
		if err != nil {
			logger.Log("err", err)
			os.Exit(1)
		}
