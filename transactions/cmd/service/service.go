...
var fs = flag.NewFlagSet("transactions", flag.ExitOnError)
var debugAddr = fs.String("debug-addr", ":5000", "Debug and metrics listen address")
var httpAddr = fs.String("http-addr", ":5001", "HTTP listen address")
var grpcAddr = fs.String("grpc-addr", ":5002", "gRPC listen address")
var thriftAddr = fs.String("thrift-addr", ":5003", "Thrift listen address")
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		endpoint, err := zipkingo.NewEndpoint("transactions", "localhost:80")
		if err != nil {
			logger.Log("err", err)
			os.Exit(1)
		}
